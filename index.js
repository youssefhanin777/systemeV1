...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'lock') {
        if (!channel.permissionsFor(interaction.guild.me).has([Permissions.FLAGS.MANAGE_CHANNELS])) {
            await interaction.reply('I do not have permission to manage channels!');
            return;
        }
